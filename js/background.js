chrome.extension.onRequest.addListener(
    function (request, sender, sendResponse) {
        ...
        var curPage = request.curPage,
            number = (localStorage.getItem('proNumber') || 10) - 0,//线程数量
            epArr = [],
            ep = new EventProxy(),
            epIndex = 0;
        ...
        epArr.push(function () {
            ...
            sendResponse({htmls: args, curPage: curPage});
            ep = null;
        });

        //执行方法
        ep.all.apply(ep, epArr);

        for (var i = 0; i < number; i++) {
            getRequest(request.requestUrl + (curPage++) + 0, function (responseText) {
                //正则过滤html
                var html = responseText.replace(/[\t\r\n ]{2,}|[\t\r\n]/g, ' ');
                html = html.match(/<body.*?>(.*?)<\/body>/ig)[0].replace(/<body.*?>(.*?)<\/body>/ig, "$1");
                html = html.replace(/<script.*?>(.*?)<\/script>/ig, "");
                html = html.replace(/<img.*?>/ig, "");
                ep.emit("t" + (epIndex++), html);
            });
        }
    });
